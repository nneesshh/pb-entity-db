 ...
 * ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â  PTHREAD_MUTEX_STALLED
 ...
 * ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â  PTHREAD_MUTEX_ROBUST
 ...
 * EOWNERDEAD.
 * Note ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Your application must always check the return code from pthread_mutex_lock() for
 * a mutex initialized with the PTHREAD_MUTEX_ROBUST attribute.
 * ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â  The new owner of this mutex should make the state protected by the mutex consistent.
 * This state might have been left inconsistent when the previous owner terminated.
 * ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â  If the new owner is able to make the state consistent, call
 ...
 * ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â  If the new owner is not able to make the state consistent, do not call
 ...
 * ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â  If the thread that acquires the lock with EOWNERDEAD terminates without unlocking the
