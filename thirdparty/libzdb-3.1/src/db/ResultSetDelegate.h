...
typedef struct Rop_S {
        ...
        int (*next)(T R);
		int(*nextResultSet)(T R);
        int (*isnull)(T R, int columnIndex);
        const char *(*getString)(T R, int columnIndex, int *outColumnFieldType);
		float (*getFloat)(T R, int columnIndex, int *outColumnFieldType);
		double (*getDouble)(T R, int columnIndex, int *outColumnFieldType);
        const void *(*getBlob)(T R, int columnIndex, int *size, int *outColumnFieldType);
        time_t (*getTimestamp)(T R, int columnIndex, int *outColumnFieldType);
        struct tm *(*getDateTime)(T R, int columnIndex, struct tm *tm, int *outColumnFieldType);
} *Rop_T;
