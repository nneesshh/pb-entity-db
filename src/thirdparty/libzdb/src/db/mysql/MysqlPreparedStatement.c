...
#include "db/PreparedStatementDelegate.h"
...
T MysqlPreparedStatement_new(const char *sql, void *stmt, int maxRows, int parameterCount) {
        T P;
...
void MysqlPreparedStatement_setString(T P, int parameterIndex, const char *x, size_t size) {
        Assertion_assert(P);
...
#define CHINA_TZ 28800
		...
		/* gmtime_r no timezone, localtime_r use timezone */
/*#ifdef _WIN32
		localtime_s(&ts, &x);
#else
		localtime_r(&x, &ts);
...
void MysqlPreparedStatement_setBlob(T P, int parameterIndex, const void *x, size_t size) {
        Assertion_assert(P);
        int i = checkAndSetParameterIndex(parameterIndex, P->parameterCount);
...
ResultSet_T MysqlPreparedStatement_executeQuery(T P) {
        ...
        mysql_stmt_attr_set(P->stmt, STMT_ATTR_CURSOR_TYPE, &cursor);
#endif
		unsigned long prefetch_rows = 10;
		mysql_stmt_attr_set(P->stmt, STMT_ATTR_PREFETCH_ROWS,
			(void*)&prefetch_rows);
        if ((P->lastError = mysql_stmt_execute(P->stmt)))
                THROW(SQLException, "%s", mysql_stmt_error(P->stmt));
        if (P->lastError == MYSQL_OK)
