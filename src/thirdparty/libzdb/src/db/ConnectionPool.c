...
void ConnectionPool_setInitialConnections(T P, int connections) {
        Assertion_assert(P);
        Assertion_assert(connections >= 0);
        LOCK(P->mutex)
        {
...
void ConnectionPool_setMaxConnections(T P, int maxConnections) {
        Assertion_assert(P);
        Assertion_assert(P->initialConnections <= maxConnections);
        LOCK(P->mutex)
        {
...
void ConnectionPool_setConnectionTimeout(T P, int connectionTimeout) {
        Assertion_assert(P);
        Assertion_assert(connectionTimeout > 0);
...
void ConnectionPool_setReaper(T P, int sweepInterval) {
        Assertion_assert(P);
        Assertion_assert(sweepInterval>0);
        LOCK(P->mutex)
        {
                P->doSweep = true;
	...
	LOCK(P->mutex) 
        {
                ...
                for (int i = 0; i < size; i++) {
                        con = Vector_get(P->pool, i);
 						available = Connection_isAvailable(con);
// 						if (available)
// 							ping = Connection_ping(con);
// 						
//                      if (available && ping) {
                        if (available) {
